...
class DigitClassifierFactoryImpl(
    private val repositoryProvider: () -> ModelWeightsRepository,
    ...
    override fun create(modelId: ModelId): DigitClassifier = when (modelId.value) {
        ModelId.CNN_MNIST.value -> CnnDigitClassifier(cnnModuleProvider(), repositoryProvider())
        ModelId.MLP_MNIST.value -> MlpDigitClassifier(mlpModuleProvider(), repositoryProvider())
